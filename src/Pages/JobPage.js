import React from 'react';
import CoachCard from '../Components/CoachCard.js';
import IconButton from '@material-ui/core/IconButton';
import SearchIcon from '@material-ui/icons/Search';
import Link from '@material-ui/core/Link';
import Grid from '@material-ui/core/Grid';
import Box from '@material-ui/core/Box';
import Button from '@material-ui/core/Button';
import { createMuiTheme, ThemeProvider } from '@material-ui/core/styles';
import { blue } from '@material-ui/core/colors';
import AppBar from '@material-ui/core/AppBar';
import MessageIcon from '@material-ui/icons/Message';
import NotificationsOutlinedIcon from '@material-ui/icons/NotificationsOutlined';
import InputBase from '@material-ui/core/InputBase';
import InputLabel from '@material-ui/core/InputLabel';
import MenuItem from '@material-ui/core/MenuItem';
import FormControl from '@material-ui/core/FormControl';
import Select from '@material-ui/core/Select';

export default function JobPage() {
    const [age] = React.useState('');
    const theme = createMuiTheme({
        palette: {
            primary: blue,
        },

    });
    const data = [
        {
            header: " Looking for certified coach in stress management", title: "Australia & New Zealand Banking Group Ltd",
            jobtype: "fulltime", facility: "mental health", facility2: "mental fitness", apply: " Apply by 30 nov 2020", money: "$1200-$2000", members: " session for 4 members"
        },
        {
            header: " Need team communication coach for a workshop", title: "Brambies ltd",
            jobtype: "fulltime", facility: "mental health", facility2: "mental fitness", apply: " Apply by 30 nov 2020", money: "$1200-$2000", members: " session for 4 members"
        },
        {
            header: " Coach to guide on a diet for Hyperbrian activity",
            title: "Bunnings Warehouse",
            jobtype: "fulltime", facility: "mental health", facility2: "mental fitness", apply: " Apply by 30 nov 2020",
            money: "$1200-$2000", members: " session for 4 members"
        },
        {
            header: " Stress management expert",
            title: "Lendlease",
            jobtype: "fulltime", facility: "mental health", facility2: "mental fitness", apply: " Apply by 30 nov 2020",
            money: "$1200-$2000", members: " session for 4 members"
        },

    ];
    return (
        <div style={{ marginTop: "2%" }}>

            <AppBar style={{ backgroundColor: "white", marginBottom: '1%' }} position="static">
                <Box display="flex" justifyContent="flex-end" >
                    <Box p={3}>
                        <Link style={{ color: "black" }} href="#">
                            Dashboard
        </Link>
                    </Box>
                    <Box p={3}>
                        <Link style={{ color: "black" }} href="#">
                            My jobs
        </Link>
                    </Box>
                    <Box p={2}>
                        <IconButton
                            edge="end"
                            aria-label="account of current user"
                        >
                            < MessageIcon />
                        </IconButton>
                    </Box>
                    <Box p={2}>
                        <IconButton
                            edge="end"
                            aria-label="account of current user"
                        >
                            < NotificationsOutlinedIcon />
                        </IconButton>
                    </Box>
                    <Box p={2}>
                        <img style={{ width: 30, marginTop: 10 }}
                            src="data:image/jpeg;base64,"
                            alt="new"
                        />
                    </Box>
                </Box>

            </AppBar>
            <Grid container spacing={0}>
                <Grid item xs={11}>
                    <p style={{ marginLeft: "4%", fontSize: "35px" }}>18 jobs posted by you</p>
                    <p style={{ marginLeft: "4%", fontSize: "19px" }}>18 jobs that you posted,coach responses and tracking  </p>
                </Grid>

                <Grid item xs={1}>
                    <ThemeProvider style={{ marginTop: '55%' }} theme={theme}>
                        <Button style={{ backgroundColor: "#FF8E31", marginTop: '1%' }} variant="contained"  >
                            Post a job
        </Button></ThemeProvider>
                </Grid>
            </Grid>
            <Box display="flex" justifyContent="flex-end" >
                <Box p={2}>
                    <Link style={{ color: "#FF8E31" }} href="#">
                        Open jobs
        </Link>
                </Box>
                <Box p={2}>
                    <Link href="#">
                        Draft jobs
        </Link>
                </Box>
                <Box p={2} >
                    <Link href="#">
                        Closed jobs
        </Link>
                </Box>
            </Box>
            <hr />
            <Grid container spacing={0}>
                <Grid item xs={6}>
                    <InputBase style={{ backGroundColor: "#2DABFF30", marginLeft: "5%" }}
                        variant="outlined"
                        placeholder="Search jobs and responses"
                    />
                    <IconButton type="submit" aria-label="search">
                        <SearchIcon />
                    </IconButton>
                </Grid>
                <Grid item xs={1}>
                    <Link href="#">
                        Advance Search
        </Link>
                </Grid>
                <Grid item xs={2}>
                    <Link style={{ float: "right", color: "#858A9A" }} href="#">
                        sort by
        </Link>
                </Grid>
                <Grid item xs={2}>
                    <FormControl style={{ float: "right", color: "#858A9A", width: "70%" }} variant="filled">
                        <InputLabel id="demo-simple-select-filled-label">Date posted</InputLabel>
                        <Select
                            labelId="demo-simple-select-filled-label"
                            id="demo-simple-select-filled"
                            value={age}

                        >
                            <MenuItem value="">
                                <em>None</em>
                            </MenuItem>
                            <MenuItem value={10}>Ten</MenuItem>
                            <MenuItem value={20}>Twenty</MenuItem>
                            <MenuItem value={30}>Thirty</MenuItem>
                        </Select>
                    </FormControl>
                </Grid>

            </Grid>
            {/* <TextField style={{width:800,marginLeft:"4%"}}
          id="filled-name"
          label="Search"
          variant="filled"
        />
       <IconButton  type="submit"  aria-label="search">
        <SearchIcon style={{marginRight:"500%"}} />
      </IconButton> */}
            {data.map((e, i) => (
                <CoachCard
                    key={i}
                    coach={e}
                />
            ))}
        </div >

    );
}